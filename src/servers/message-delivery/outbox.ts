...
import { MessageDispatchFactory } from "./message-dispatch-factory.js";
import { MessageDispatch, MessageDispatchType } from "./message-dispatch.js";
...
export class MessageDispatchOutbox<Sendable> {
  ...
  removeRecipients(connectionIds: ConnectionId[]) {
    ...
    for (const dispatch of this.list) {
      if (dispatch.type === MessageDispatchType.Single) {
        ...
        dispatch.connectionIds = dispatch.connectionIds.filter(
  ...
  pushToChannel(
    ...
    this.list.push(
      this.dispatchFactory.createFanOut(inChannel, update, options)
    );
  }
