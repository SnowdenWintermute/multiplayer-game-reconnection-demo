...
export class GameSessionStoreService {
  ...
  async getPendingGameSetup(
    gameName: GameName
  ): Promise<PendingGameSetup | null> {
  ...
  async writeActiveGameStatus(
    gameName: GameName,
    game: ActiveGameStatus
  ): Promise<void> {
    this.activeGameStatusRecords.set(gameName, game);
  }

  async getActiveGameStatus(
    gameName: GameName
  ): Promise<ActiveGameStatus | null> {
