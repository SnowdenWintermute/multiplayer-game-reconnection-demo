...
export class UserSessionRegistry extends ConnectionSessionRegistry<UserSession> {
  ...
  requireSingleConnection(userId: UserId) {
    ...
    invariant(connectionIdOption !== undefined);
  ...
  getExpectedUserSessions(userId: UserId) {
    ...
    for (const id of connectionIds) {
      ...
      invariant(
        ...
        "A connection id was registered without a matching session"
      );
  ...
  private addConnectionIdToUserId(session: UserSession) {
    const connectionIdsForThisUser = this.connectionIdsByUserId.get(
      session.taggedUserId.id
    );
    if (connectionIdsForThisUser) {
      connectionIdsForThisUser.add(session.connectionId);
    } else {
      this.connectionIdsByUserId.set(
        session.taggedUserId.id,
        new Set([session.connectionId])
      );
    }
  }
  ...
  private removeConnectionIdFromUserId(session: UserSession) {
    ...
    invariant(connectionIdsForThisUser !== undefined);
  ...
  public getExpectedSessionInGameByUsername(
    username: Username,
    gameName: GameName
  ) {
    ...
    invariant(expectedSessionForThisPlayer !== undefined);
